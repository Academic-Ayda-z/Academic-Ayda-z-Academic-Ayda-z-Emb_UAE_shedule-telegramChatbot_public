...
def set_user_schedule_id(user, schedual_id):
  ...
  all_users[user.user_id].schedual_id = schedual_id


def add_user(desired_date, chat_id, email_add, password):
  global all_users
  new_user = User(chat_id, desired_date)
  all_users[chat_id] = new_user
  all_users[chat_id].account_password = password
...
def SCHEDUAL_APPOINTMENT_SUBMIT(driver):
  ...
  if attempt == 0:
    ...
    with open("schedule_count.txt", 'w') as schedule_appointment:
...
def go_to_prev_month_calender(driver):
  ...
  current_calender_month = months_dict[calender_month.text]
  print(
    f"Now we are in month {current_calender_month} aka {calender_month.text}",
    end='\n')
  return current_calender_month
