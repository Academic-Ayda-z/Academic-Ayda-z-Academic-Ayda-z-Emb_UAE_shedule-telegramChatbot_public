...
def set_date(dates):
  ...
  if datetime.strptime(str(dates[1]), '%Y-%m-%d') < today:
    print("the first date must be today or later!")
    return []
  ...
  if get_desired_num_months(dates) > 14:
...
class User:
  ...
  def __init__(self, chat_id, desired_dates):
    ...
    self.location = ""
    self.schedual_id = ""
    self.desired_dates = set_date(desired_dates)
    self.desired_num_months = get_desired_num_months(self.desired_dates)
